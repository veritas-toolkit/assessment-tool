...
def waterfall(base_values, loc, values, features, feature_names, filename, max_display=10, show=True):
    ...
    plt.barh(pos_inds, np.array(pos_widths) + label_padding + 0.02*dataw,
             left=np.array(pos_lefts) - 0.01*dataw, color='#FF0051', alpha=0)
    label_padding = np.array([-0.1*dataw if -w < 1 else 0 for w in neg_widths])
    plt.barh(neg_inds, np.array(neg_widths) + label_padding - 0.02*dataw,
             left=np.array(neg_lefts) + 0.01*dataw, color='#008BFB', alpha=0)
    ...
    for i in range(len(pos_inds)):
        ...
        if text_bbox.width > arrow_bbox.width:
            ...
            txt_obj = plt.text(
                ...
                horizontalalignment='left', verticalalignment='center', color='#FF0051',
                fontsize=12
            )
    ...
    for i in range(len(neg_inds)):
        ...
        if text_bbox.width > arrow_bbox.width:
            ...
            txt_obj = plt.text(
                ...
                horizontalalignment='right', verticalalignment='center', color='#008BFB',
                fontsize=12
            )
