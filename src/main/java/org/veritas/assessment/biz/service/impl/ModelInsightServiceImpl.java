...
public class ModelInsightServiceImpl implements ModelInsightService {
    ...
    public void autoGenerateAnswer(Project project, ModelArtifact modelArtifact) {
        ...
        final List<String> partList = Arrays.asList("B", "C", "D");
        ProjectQuestionnaire questionnaire = questionnaireService.findByProject(project.getId());


        List<ProjectQuestion> updateQuestionList = new ArrayList<>();
        for (ProjectQuestion question : questionnaire.getQuestions()) {
            if (partList.contains(question.getPart())) {
                boolean autoGenerated = answerContent(modelMap, project.getBusinessScenario(), question);
                for (ProjectQuestion subQuestion : question.getSubQuestions()) {
                    boolean autoSub = answerContent(modelMap, project.getBusinessScenario(), subQuestion);
                    autoGenerated = autoGenerated || autoSub;
                }
                if (autoGenerated) {
        ...
        questionnaireService.forceUpdateAnswerList(project.getId(), updateQuestionList);
    }
...
//        GraphContainer container = graphService.createAllGraph(jsonModel);
//        GraphContainer container = graphService.createAllGraph(project, jsonModel);
