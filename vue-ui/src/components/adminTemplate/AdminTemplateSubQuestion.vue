...
export default {
  ...
  methods: {
    getSubQuestion() {
      this.$http.get(`/api/admin/questionnaire/${this.templateId}/question/${this.questionId}`).then(res => {
        if (res.status == 200) {
    ...
    addSubQuestion() {
      let newSubQues = {}
      newSubQues.templateId = this.templateId
      newSubQues.mainQuestionId = this.questionId;
      newSubQues.question = this.addSubQues
      // newSubQues.subSerial = 1
      this.$http.post(`/api/admin/questionnaire/${this.templateId}/question/${this.questionId}/sub/new`,newSubQues).then(res => {
        if (res.status == 200) {
          this.subQuestionList = res.data.subQuestionList
          this.addSubQues = ''
          this.$emit('updateFlag', false)
    ...
    writeSubQues(id,vid) {
      let editSubQues = {}
      editSubQues.id = id
      editSubQues.mainQuestionId = this.questionId
      editSubQues.content = this.editSubQues[id]
      this.$http.post(`/api/admin/questionnaire/${this.templateId}/question/${this.questionId}/sub`,editSubQues).then(res => {
        if (res.status == 200) {
          this.subQuestionList = res.data.subQuestionList
    ...
    upRecordSubQues(index) {
      ...
      subQuestionReorder.templateId = this.templateId
      ...
      subQuestionReorder.newOrderList = subQuesList
      this.$http.post(`/api/admin/questionnaire/${this.templateId}/question/${this.questionId}/sub/reorder`,subQuestionReorder).then(res => {
        if (res.status == 200) {
          this.subQuestionList = res.data.subQuestionList
        }
      })
